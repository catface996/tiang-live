...
import { Card, Space, Button, Tag, Avatar, Tooltip, Dropdown, Typography, Statistic } from 'antd';
import {
  FileTextOutlined,
  EyeOutlined,
  EditOutlined,
...
const StyledCard = styled(Card)<{ $isDark: boolean }>`
  ...
  &:hover {
    box-shadow: ${props => (props.$isDark ? '0 4px 12px rgba(255, 255, 255, 0.1)' : '0 4px 12px rgba(0, 0, 0, 0.15)')};
...
const ReportCard: React.FC<ReportCardProps> = ({ report, onView, onEdit, onDownload, onDelete }) => {
  ...
  return (
    <StyledCard
      ...
      actions={[
        <Tooltip title={t('common:view')} key="view">
          <Button type="text" icon={<EyeOutlined />} onClick={e => handleAction('view', e)} />
        </Tooltip>,
        <Tooltip title={t('common:edit')} key="edit">
          <Button type="text" icon={<EditOutlined />} onClick={e => handleAction('edit', e)} />
        </Tooltip>,
        <Tooltip title={t('common:download')} key="download">
          <Button type="text" icon={<DownloadOutlined />} onClick={e => handleAction('download', e)} />
        </Tooltip>,
        <Dropdown
          menu={{
    ...
    >
      <div className="report-header">
        <div className="report-icon">
          <Avatar
            ...
            style={{
              backgroundColor:
                typeConfig.color === 'blue' || typeConfig.color === '#1890ff'
                  ? '#1890ff'
                  : typeConfig.color === 'purple' || typeConfig.color === '#722ed1'
                    ? '#722ed1'
                    : typeConfig.color === 'cyan' || typeConfig.color === '#13c2c2'
                      ? '#13c2c2'
                      : typeConfig.color === 'gold' || typeConfig.color === '#faad14'
                        ? '#faad14'
                        : '#1890ff'
            }}
          />
      ...
      <div className="report-stats">
        <Space split={<span style={{ color: isDark ? '#434343' : '#d9d9d9' }}>|</span>} size="small" wrap>
          <Statistic
            title={
              <span style={{ color: isDark ? '#8c8c8c' : '#666', fontSize: 11 }}>{t('reports:card.fileSize')}</span>
            }
            value={report.size}
            valueStyle={{
              fontSize: 13,
          ...
          <Statistic
            title={
              <span style={{ color: isDark ? '#8c8c8c' : '#666', fontSize: 11 }}>{t('reports:card.downloads')}</span>
            }
