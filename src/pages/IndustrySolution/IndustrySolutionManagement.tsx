...
const IndustrySolutionManagement: React.FC = () => {
  ...
  const industryMap = {
    ecommerce: { name: '电商零售', color: 'blue', icon: <ShopOutlined /> },
    finance: { name: '金融银行', color: 'green', icon: <BankOutlined /> },
    healthcare: { name: '医疗健康', color: 'red', icon: <MedicineBoxOutlined /> },
    automotive: { name: '汽车制造', color: 'orange', icon: <CarOutlined /> },
    education: { name: '教育培训', color: 'purple', icon: <BookOutlined /> },
    logistics: { name: '物流运输', color: 'cyan', icon: <GlobalOutlined /> },
  };
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            ...
            <Paragraph style={{ marginTop: 8, marginBottom: 0, fontSize: 16 }}>
              预定义的行业业务场景、实体模型和关系架构，快速构建行业解决方案。
            </Paragraph>
          </div>
          <Space>
            ...
            <Col xs={24} sm={12} lg={8} xl={6} key={solution.id}>
              ...
              >
                ...
                <div style={{ fontSize: 12, color: '#666' }}>
                  <div>创建者: {solution.createdBy}</div>
                  <div>更新时间: {solution.lastModified}</div>
                </div>
              </SolutionCard>
            </Col>
          );
