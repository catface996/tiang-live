...
const HookTasks: React.FC = () => {
  ...
  const hookTypeMap = {
    webhook: { name: 'Webhook', color: 'blue', icon: <LinkOutlined /> },
    database: { name: '数据库', color: 'green', icon: <DatabaseOutlined /> },
    message_queue: { name: '消息队列', color: 'orange', icon: <BellOutlined /> },
    api_call: { name: 'API调用', color: 'purple', icon: <ApiOutlined /> },
    script: { name: '脚本执行', color: 'cyan', icon: <CodeOutlined /> },
  };
  ...
  const handleToggleHook = (hookId: string, currentStatus: string) => {
    ...
    message.success(`Hook任务已${newStatus === 'active' ? '启用' : '停用'}`);
  };

  const handleDeleteHook = (hookId: string) => {
    message.success('Hook任务删除成功');
  };
  ...
  const handleModalOk = async () => {
    try {
      ...
      if (editingHook) {
        message.success('Hook任务更新成功');
      } else {
        message.success('Hook任务创建成功');
      }
  ...
  return (
    <PageContainer>
              ...
              <HookCard
                ...
                extra={
                  <Space>
                    ...
                    <Tooltip title={hook.status === 'active' ? '停用' : '启用'}>
                      <Button 
                        type="link" 
      ...
      {/* 创建/编辑Hook任务模态框 */}
      <Modal
        title={editingHook ? '编辑Hook任务' : '创建Hook任务'}
        open={modalVisible}
        onOk={handleModalOk}
      ...
      >
        ...
        >
          ...
          >
            <Input placeholder="请输入Hook任务名称" />
          </Form.Item>

          <Form.Item
            name="description"
            label="描述"
            rules={[{ required: true, message: '请输入Hook描述' }]}
          >
            <TextArea 
              rows={3} 
              placeholder="请输入Hook任务的功能描述"
            />
          </Form.Item>
